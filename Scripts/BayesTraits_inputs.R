...
write.table(data.frame(Files=list_files), 'Data/BayesTrait_ListFiles.txt', quote = F, sep = '\t', col.names = F, row.names = F)

length(variable_AAsites) == length(list_per_pos_AA_stats)
nrow(AA_ambiguous_df_shallow_deep_allelic_var_stats) == length(list_files)
###
